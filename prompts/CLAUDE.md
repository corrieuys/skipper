...
| `commands-messages.md` | When/how to post an operator message (`post_message`). Appended to every agent prompt (root, delegated, solo); the tool is always registered. Opens with the rule that plain output never reaches the operator: any reply to operator input, and any requested summary/status, IS a `post_message` carrying the content itself |
...
| `glyph.md` | Glyph renderer agent system prompt (experimental): the one-way protocol subset, the fixed skeleton ids, and how to deduce decisions / open questions / needs-you from notes, messages, artifacts and escalations. Reply contract: one ```glyph block with RENDER / PATCH / NOOP. See [../src/glyph/CLAUDE.md](../src/glyph/CLAUDE.md) |
