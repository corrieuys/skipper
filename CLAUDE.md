...
bun run gen:icons                 # regen the embedded Lucide icon set (public/lucide-icons.json) from lucide-static, then gen:assets
...
| per-task memory (recorded exchanges + embeddings, `query_task_memory`; managed local llama-server) | [src/task-memory/CLAUDE.md](src/task-memory/CLAUDE.md) |
