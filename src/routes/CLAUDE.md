...
| `pages.ts` | Server-rendered HTML pages + polling fragments. Dashboard, tasks, escalations, logs, events, config. Teams live on `/teams`, `/teams/new`, `/teams/:id` (index grid + interactive team map). The old config-page team form is gone; `/config/teams/new`, `/config/teams/:id/edit` and `/local-teams` redirect to the new pages |
