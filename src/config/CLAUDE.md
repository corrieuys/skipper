...
| `model-settings.ts` | Machine-scoped provider+model overrides (config page "Agent Models"): skipper, greg, dictation (experimental). `PROVIDER_ALLOWLIST` gates which agent types are selectable; `codex`, `opencode`, and `grok` are experimental providers, selectable only when `isExperimental()` |
