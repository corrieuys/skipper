...
`post_message` (experimental) records a short, plain-language update for the human operator (`src/messages/`). Registered on root AND delegated sessions — any agent can notice something worth surfacing. One-way: nothing reads messages back into a prompt, so there is no list/get counterpart. Writing rules live in `prompts/commands-messages.md`.
