...
describe("Legacy schema migration", () => {
  ...
  it("drops legacy team_agents.skills column by rebuilding table", () => {
    ...
    try {
      ...
      const row = legacyDb
        .prepare("SELECT role, level, max_complexity FROM team_agents WHERE id = 'ta1'")
        .get() as { role: string; level: number; max_complexity: number } | null;
      expect(row).not.toBeNull();
      expect(row?.role).toBe("worker");
      expect(row?.level).toBe(1);
      expect(row?.max_complexity).toBe(7);
    } finally {
      legacyDb.close();
      try { unlinkSync(legacyDbPath); } catch {}
    }
