...
export function dashboardNotesFragment(notes: TaskNoteData[], taskId?: string): string {
    ...
    <form id="${formId}" class="note-add-form"
          ...
          style="margin-bottom:0.75rem;display:flex;flex-direction:row;align-items:stretch;gap:0.4rem;">
      <textarea id="${textareaId}" name="content"
                placeholder="Add a note for agents..."
                rows="1"
                style="flex:1;min-width:0;resize:vertical;font-size:0.82rem;padding:0.35rem 0.5rem;border:1px solid var(--sk-border-subtle);border-radius:var(--sk-radius-sm);background:color-mix(in srgb, var(--sk-surface-0) 28%, transparent);color:var(--on-surface);outline:none;"></textarea>
      <button type="submit"
              style="flex:0 0 auto;font-size:0.78rem;padding:0.25rem 0.85rem;">Add Note</button>
    </form>` : "";
