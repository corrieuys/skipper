...
describe("resolveEscalation", () => {
  ...
  it("attempts resume when agent is not running", async () => {
    ...
    expect(resumeSpy).toHaveBeenCalledWith(agentId, "[USER_RESPONSE] Answer", false);
  });

  it("resumes the runtime instance that raised the escalation", async () => {
    setupAgentType("resumable", true, true);
    db.prepare("UPDATE agent_types SET resume_flag = '--resume' WHERE name = 'resumable'").run();
    const agentId = createAgent("agent-resume-runtime", "resumable");
    const taskId = createRunningTask(agentId);
    const runtimeId = "runtime-escalator-1";

    db.prepare(
      `INSERT INTO agent_instances (id, task_id, template_agent_id, status, session_id)
       VALUES (?, ?, ?, 'stopped', 'sess-rt-123')`,
    ...
    await escalationManager.resolveEscalation(esc.id, "Runtime answer");
