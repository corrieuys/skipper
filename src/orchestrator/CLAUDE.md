...
| `delegation-manager.ts` | Delegation signals. Spawn child instance/batch. Track groups. Resume parent on complete. Limits: depth 3, per-parent 20, batch 8. Timeout 60min. Optional per-delegation working directory (`delegate`'s `working_directory` → `delegations.working_directory`): sets the child's `WORKING DIRECTORY:` prompt line **and** its spawn cwd — the only case besides a consensus worktree where a child does not start in the orchestrator's cwd. `validateWorkingDirectory()` (exported here) requires an absolute path that exists; a retry/resume replays the stored value |
