...
describe("truncateResult", () => {
  ...
  it("truncates text exceeding the limit", () => {
    const input = `${"a".repeat(60)}${"b".repeat(40)}`;
    const result = truncateResult(input, 90);
  ...
  it("includes truncation marker with character count", () => {
    ...
    expect(result).toContain("[truncated — result exceeded 100 characters; showing start and end]");
  });
});
...
describe("delegation result truncation", () => {
  ...
  describe("handleChildExit", () => {
    ...
    it("prefers final result payload over raw terminal JSON stream", () => {
      db.prepare(
        `INSERT INTO delegations (id, parent_agent_id, child_agent_id, task_id, prompt, status)
         VALUES ('del-2', 'parent-1', 'child-1', 'task-1', 'do something', 'running')`,
      ).run();

      db.prepare(
        `INSERT INTO terminal_outputs (agent_id, stream, sequence, data)
         VALUES ('child-1', 'stdout', 1, ?)`,
      ).run(`{"type":"system","subtype":"init","session_id":"abc"}`);
      db.prepare(
        `INSERT INTO terminal_outputs (agent_id, stream, sequence, data)
         VALUES ('child-1', 'stdout', 2, ?)`,
      ).run(`{"type":"assistant","message":{"content":[{"type":"text","text":"working"}]}}`);
      db.prepare(
        `INSERT INTO terminal_outputs (agent_id, stream, sequence, data)
         VALUES ('child-1', 'stdout', 3, ?)`,
      ).run(`{"type":"result","result":"Final concise summary"}`);
      ...
      expect(sentMessage).toContain("Final concise summary");
      expect(sentMessage).not.toContain(`"subtype":"init"`);
    });
  });
